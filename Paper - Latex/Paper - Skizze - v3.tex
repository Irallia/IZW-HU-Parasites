    ...
    \section{Ancestral state reconstruction methods}
      ...
      \subsection{Fitch maximum parsimony}
        ...
        Note, the original Fitch algorithm has the sole purpose of minimizing the number of transitions 
          ...
          these. \todotext{Wir haben mit ein paar kleinen änderungen optimiert... und schließlich auf multifurcated angepasst...} \\
        ...
        Input: A rooted, binary tree, with state informations in the leaf nodes. Each node is depicted as 
          a set of states. Internal nodes have three sets, which are empty at the beginning, excluding the
          root node, it has only one. Leaf nodes have their state as a set (eg \{FL\}, unknown leaf 
          nodes) the union of all possible states (\{FL, P\}). \\
        ...
        \begin{wrapfigure}{r}{0.4\textwidth}
          \centering
          \includegraphics[width=0.39\textwidth]{Figures/Fitch1.png}
        ...
        \end{enumerate}
        First traverse from the leaf nodes to the root / move down the tree / postorder tree traversal. Each internal node is formed from its child nodes. \\
        Second traverse from the root node to the leafs. Each internal node is formed from its father node 
          and its sibling node. \\
        Last traversion (direction does not matter \todotext{does it? Welche Mengen/Menge wird genommen 
          von jedem Knoten?}): Build the final state for every node. It is formed 
          from the sets of previous traversals. \\
        
        Figure \ref{fig: binary Fitch} shows the firch algorithm for binary trees with 5 leaf nodes, \todo{one unknown}.

        The finale step is not part of the original Fitch algorithm, as it only aims to minimize the 
          transitions without predicting actual states of internal nodes. This step is described by \todo{...}. \\
      ...
      \begin{table}[h!]
        \begin{center}
          \begin{tabular}{ |l|r|r|r|r| }
            ...
            \hline \hline
            multifurc $\sim$ taxa & 545799 & \cellcolor{green!10}500004 & \cellcolor{green!50}485121 \\
            \hline
            multifurc $\sim$ taxa + depth & 544862 & \cellcolor{green!30}493808 & \cellcolor{green!50}481869 & 478851 \\
            \hline
            multifurc $\sim$ taxa * depth & 544179 & \cellcolor{green!50}489845 & \cellcolor{green!50}481494 \\
            \hline
          \end{tabular} 
        \end{center}
