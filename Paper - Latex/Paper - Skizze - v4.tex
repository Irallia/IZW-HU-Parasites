...
\chapter{Introduction}
  ...
  We will test the existing maximum parsimony algorithms Fitch \cite{Fitch1971} and Sankoff 
    \cite{Sankoff1975} for this task and estimate their predictive power. The present tree of OTL is 
    not binary. A tree is binary (or bifurcated), when every node has at most two children otherwise 
    it is multifurcated \cite{Felsenstein2003}. \\
  ...
  For the information about the current states of the species we use the interaction database Global 
    Biotic Interactions (GloBI) \cite{Poelen2014}. The data in GloBI are stored as interactions. 
    Useful for us e.g. Species A parasitize species B. From this we conclude that species A is 
    parasitic and species B free-living. In the case parasite parasitizes parasite, we get conflicting 
    states for the second species. We solve this by preferring parasitic. \\

  At this point a few words to the term parasitic. There are different definitions. Since we use Globi 
    to classify species, we use their definition of parasitism. Again, in GloBi, ontobee definitions 
    are used.
  The interaction has parasite is defined as: "An interaction relationship between two organisms 
    ...
    disadvantageous or destructive to one of the organisms." \cite{...}. This definition includes: 
    ecto- and endoparasites, parasitoids, kleptoparasites and pathogenes.
  ...
  This paper thus consists of the following points: (1) Find a suitable ancestral state 
    reconstruction method. (2) Perform reconstructing on the Eucaryotic synthesis tree of OTL. \\
  Point 1 consists of a simulation of our data situation and the application of various possible 
    algorithms to these. To evaluate them based on the results. \\
  The one by Louca et al. implemented Sankoff algorithm is the best in our comparisons. Therefore, 
    ...
    And then an evaluation of the results. \\

  \todotext{Definitions of height (min, max, mean), depth of a tree/node (Distanz zur Wurzel vs 
    distanz zum Blatt)}
