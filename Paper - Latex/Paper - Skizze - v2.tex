...
%--------------------------------------------------------------------------------------------------- 
\chapter{Methods}
  We would like to apply a maximum parsimony algorithm to the entire tree of life to obtain an 
    ancestral state reconstruction of free-living vs. parasite. On one side we need a simulation to
    find out, which is the best algorithm with which parameters. And on the other side we run the 
    selected Algorithm and evaluate the results.\\
  \begin{figure}[h!]
    \caption{Workflow}
    \centering
    \label{fig:Workflow}
  ...
  \end{figure}
  For this we have to look at the following different aspects:
  \begin{enumerate}
    \item Get the real tree and real data for the leaf nodes. $\rightarrow$ OTL, GloBI
