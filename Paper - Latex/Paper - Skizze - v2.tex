...
%--------------------------------------------------------------------------------------------------- 
\chapter*{Abstract}
  With increasing amounts of data, new investigations are also possible in phylogeny. This work 
    deals with the ancestral state reconstruction of parasitism in the tree of life of eukaryotes. 
    To predict states of the unknown species and estimate origins and losses of parasitism. \\
  We performed an analysis of existing algorithms and applied a Sankoff maximum parsimony algorithm 
    using the R package Castor \cite{Louca2017}. \\
  The challenge here is the size of the tree and the little information about it. On the one hand, 
    such a large phylogenetic tree is not completely existent, and we therefore work with a synthesis 
    tree of OTL \cite{Hinchliff2015}, which is highly multifurcated. On the other hand, for the 
    approximately 300 000 leaf nodes we found only for $\approx 4,3\%$ information whether they are 
    parasitic or free-living species (obtained from the GloBI database \cite{Poelen2014}).
\tableofcontents
\clearpage
